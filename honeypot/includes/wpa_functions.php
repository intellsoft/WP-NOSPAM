...
function wpa_plugin_menu(){
    add_menu_page( 'NO SPAM', 'NO SPAM', 'edit_pages', 'wp-armour', 'wpa_options','dashicons-shield');
}
...
function wpa_options(){
	$wpa_tabs = array(
				...
				'extended_version' => array('name'=>"What's in NO SPAM Extended ?",'path'=>'wpa_extended_version.php')
	);
...
function wpa_field_info(){
	$wpa_add_test = 'no'; // test widget completely removed
...
// ** NEW: Auto‑change Honey Pot field name every day **
add_action( 'init', 'wpa_maybe_update_field_name' );
function wpa_maybe_update_field_name() {
    $last_changed = get_option( 'wpa_field_name_last_changed', '' );
    $today = date( 'Ymd' );

    if ( $last_changed !== $today ) {
        update_option( 'wpa_field_name', wpa_unqiue_field_name() );
        update_option( 'wpa_field_name_last_changed', $today );
        $GLOBALS['wpa_field_name'] = get_option( 'wpa_field_name' ); // refresh for current request
    }
}